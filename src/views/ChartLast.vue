<template>
  <div class="chart">
    <v-container fluid fill-height >
      ...
      <!-- option: {{ oidx }} - meta: {{findMeta(oidx)}} -->
                    <!-- <v-text-field  v-if="docs[sidx][oidx].meta.type.includes('string') || docs[sidx][oidx].meta.type.includes('number')"
                      :label="oidx"
                      persistent-hint
                      :hint="`${option.meta.type} - ${option.meta.help}`"
                      :value="docs[sidx][oidx].meta.model"
                      @input="(newval) => {docs[sidx][oidx].meta.model = newval}">
                    </v-text-field>

                    <v-switch v-else-if="docs[sidx][oidx].meta.type.includes('boolean')"
                      :label="oidx"
                      persistent-hint
                      :hint="`${option.meta.type} - ${option.meta.help}`"
                      :value="docs[sidx][oidx].meta.model"
                    ...
                    <v-text-field
                      ...
                      persistent-hint
                      :hint="`${option.meta.type} - ${option.meta.help}`"
                      v-model="docs[sidx][oidx].meta.model">
                    </v-text-field>
...
export default {
  ...
  watch: {
    ...
    //     console.log('watcher', newval)
    //     // this.chartConfig = newval
    //     this.chartConfig.primaryValuesTitle = newval.chart.primaryValuesTitle.meta.model
    //     this.renderChart()
    //   },
    //   deep: true
    // }
  ...
  data: () => ({
    ...
    //         help: "title of the the primary values axis",
    //         model: {}
    //       }
    //     }
    //   }
  ...
  computed: {
    sections: function () {
      return Object.keys(this.docs).sort((a,b) => (a > b))
    },

    // docs: {
    //   get: function () { return this.docs },
    //   set: function (nval) {
    //     this.chartConfig.primaryValuesTitle = nval
    //     this.renderChart()
    //   }
    // }
    //
  ...
  mounted () {
    ...
    console.log('docs', this.docs)
     ...
     // this.chartConfig.primaryValuesTitle = 'hello title'
    ...
    console.log('chartConfig', self.chartConfig)

    // let testconfig = {chart: { legend: { title: 'title', months: ['Januar', 'Februar'], visible: true, margin: { right: 30, left: 50 }}}}
    // let right = new Proxy(testconfig, {
    //   get: function (target, name) {
    //     return name in target ? target[name] : 'key not defined'
    //   }
    // })
    // console.log('proxy', right.chart.legend)

    for (let section in this.docs) {
      this.walk(this.docs[section], function (parent, option, obj) {
        self.$watch(
          () => (obj),
          self.changeOption,
        { deep: true })
    ...
    console.log('watchers', self.$watch)
    console.log('mounted...', this.docs, this.chartConfig)
  ...
  methods: {
    ...
    changeOption: function (newval) {
      let item = this.findConfigOption(newval.meta.name, this.chartConfig)
      // console.log('watcher setting ', item, item[newval.meta.name])
      let path = newval.meta.name.split('.')
      console.log('Model name', newval.meta.name, path)
      item[newval.meta.name] = +newval.meta.model || newval.meta.model
      this.renderChart()
    },

    findConfigOption: function(option, settables) {
      let found
      this.walk(settables, function (parent, opt, obj) {
        console.log('findConfigOptionwalk', option, opt)
        if (opt === option) {
          found = parent
          return true
        }
      ...
      console.log('findConfigOption1', option, opt, settables) //, Object.keys(settables))
    ...
    createConfigUI: function () {
      // let config = this.docs
      let enums = {
        dataColorsScheme: ['schemeCategory10', 'schemeSet3']
      }
      for (let section in this.docs) {
        for (let option in this.docs[section]) {
          ...
          if (Object.keys(enums).includes(option)) this.docs[section][option].meta.control = 'options'
          else if (Array.isArray(object)) this.docs[section][option].meta.control = 'array'
          this.docs[section][option].meta.control = typeof object
          // this.docs[section][option].meta.model = object
        }
      }
      console.log('createConfigUI', this.docs)
    },
      ...
      if(obj && typeof obj === 'object') {
        ...
        for(let i in obj) {
          if(obj.hasOwnProperty(i)) {
            if (i !== ignore) {
              stop = callback(obj, i, obj[i])
              if (!stop) this.walk(obj[i], callback, ignore)
            }
    ...
    renderChart: function () {
      ...
      // console.log('optionsdoc', this.docs)
